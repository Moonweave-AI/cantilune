...
export class AgentInstance {
  ...
  private buildAgentSystemPrompt(): string {
    return [
      ...
      "- Growing the cluster takes three committed steps, in order:",
      "  1. `write_content` the new agent's manifest JSON; keep the returned ref.",
      "  2. `register_participant` to record it (this alone launches nothing).",
      "  3. `activate_participant` with that ref to admit it and start its loop.",
      "- A manifest declares agentId, kind, systemPrompt, assignedTask, startCondition,",
      "  heartbeatIntervalMs, designedBy, and optionally priority. Its `agentId` must equal",
      "  the participant you registered, or activation is rejected.",
      "- `startCondition` is a StartConditionExpression tree (operator/atom/evaluator), not prose.",
      "  It is re-evaluated against the committed world on every scheduler tick, so an agent that",
      "  must wait for others may declare `agentsDone` and be activated now. Missing or non-expression",
      "  values start immediately.",
      "- You are a peer, not a supervisor: any active participant may register and activate others,",
      "  and whoever designs an agent owns whether its start condition can ever be satisfied.",
