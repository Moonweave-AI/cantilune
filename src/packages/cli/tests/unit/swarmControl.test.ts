/**
 * Unit test for the CLI swarm controller (ADR-0019).
 *
 * Verifies that createSwarmController builds a real CantiluneSwarm (via
 * bootSwarm) from the handle backends and that activate() stores a manifest
 * and commits a valid activate_participant intent through the runtime. The
 * runtime is a controllable mock (real CollaborationSnapshot), the content
 * store is a mock, and the LLM adapter is a no-op stub — we are testing the
 * controller wiring, not the agent loop.
 */
...
import { createSwarmController } from "../../src/wiring/swarmControl.js";
...
describe("createSwarmController", () => {
  it("start() reports not connected when backends are absent", () => {
    const controller = createSwarmController(
  ...
  it("start() builds a real swarm and status reports running", () => {
  ...
  it("start() twice reports already running", () => {
    const store = createMockContentStore();
    const initiator = actorId("initiator");
    const snap = snapshotWithParticipants(
      new Map([[initiator, participant(initiator, "agent", "active")]]),
  ...
  it("activate() stores a manifest and commits activate_participant", async () => {
    ...
    // A manifest was stored with the swarm-activate provenance and the given task.
    ...
    expect(manifest.systemPrompt).toBe("you are a swarm worker");
  ...
  it("waitForCompletion() returns a not-started result when the swarm is not started", async () => {
    const controller = createSwarmController(
      () => ({ contentStore: undefined, syscallRuntime: undefined, storagePath: undefined }),
      stubLlmAdapter,
    );
    ...
    expect(result.ok).toBe(false);
    expect(result.summary).toContain("swarm not started");
  });

  it("shutdown() when not started is a no-op", async () => {
    const controller = createSwarmController(
      () => ({ contentStore: undefined, syscallRuntime: undefined, storagePath: undefined }),
      stubLlmAdapter,
    );
