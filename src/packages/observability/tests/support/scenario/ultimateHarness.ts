...
function runExtraIntroducePair(
  ...
  return [
    proposeAndCommitOrThrow(runtime, introduceIntent(taskA, runtimeActors.planner)).change,
    proposeAndCommitOrThrow(runtime, introduceIntent(taskB, holderB)).change,
  ];
}
...
export function runUltimateRuntimeClosure(
  ...
  for (let index = 0; index < observations; index++) {
    ...
    const observed = runtime.observe(
      { source, payloadRef: contentRef(`content://ultimate-obs-${index}`) },
      { principal: source },
    );
