...
function isProcessAlive(pid: number): boolean {
  try {
    // Signal 0 performs the permission check without delivering a signal.
    process.kill(pid, 0);
    return true;
  } catch (error) {
...
/** Owning pid recorded in the lock file, or undefined if it cannot be read. */
...
function publishLockCandidate(lockPath: string): string | undefined {
  ...
  try {
    ...
    closeSync(fd);
    fd = undefined;
    // Publish a complete inode with create-if-absent semantics. The lock path
    // can never expose the candidate's partially written contents.
