import { describe, expect, it } from "vitest";
import {
  actorId,
  actorRef,
  changeId,
  coordinationChange,
  epochId,
  matchBinding,
  operationTypeId,
  snapshotRef,
  timestamp,
  withSnapshotRef,
} from "@cantilune/core";
import { createPostgresDurableCoordinator } from "../../../src/memory/postgresDurableCoordinator.js";
import { replayRecipe } from "../../../src/replay/recipe.js";
import { buildConfigT0 } from "../../support/fixtures/config-t0.js";

/**
 * Live Postgres L7. Not part of the default `vitest run` include set.
 * CI runs `test:postgres-live` with a Postgres service. Missing URL is an
 * explicit failure — never skipIf.
 */
const url = process.env.CANTILUNE_DURABLE_DATABASE_URL?.trim();
if (url === undefined || url.length === 0) {
  throw new Error(
    "CANTILUNE_DURABLE_DATABASE_URL is required for postgres-live-fencing (ADR-0023). CI provides Postgres; locally set the URL or run the default suite.",
  );
}

describe("L7 live Postgres fencing", () => {
  const t0 = buildConfigT0();
  const recipe = replayRecipe({
    epochId: epochId("42"),
    operationTypeId: operationTypeId("introduce_artifact"),
    matchBindings: [matchBinding("task", "task-T"), matchBinding("from", "planner-p")],
    visibility: "external",
  });
  const change = coordinationChange({
    changeId: changeId(`chg-live-fence-${Date.now()}`),
    recordedAt: timestamp("2026-08-16T00:00:00Z"),
    epochId: epochId("42"),
    operationTypeId: operationTypeId("introduce_artifact"),
    beforeRef: t0.snapshotRef,
    afterRef: snapshotRef(`snap-live-fence-${Date.now()}`),
    matchBindings: recipe.matchBindings,
    initiator: actorRef(actorId("planner-p"), "agent"),
    visibility: "external",
  });
  const after = withSnapshotRef(t0, change.afterRef);

  it("fences out a stale writer against a live database", () => {
    const schema = `cantilune_live_${Date.now()}`;
    const stale = createPostgresDurableCoordinator({
      connectionString: url,
      schema,
      initial: t0,
      leaseOwner: "writer-stale",
      leaseToken: "token-stale",
    });
    const live = createPostgresDurableCoordinator({
      connectionString: url,
      schema,
      leaseOwner: "writer-live",
      leaseToken: "token-live",
    });
    expect(
      stale.commit({
        expectedHead: t0.snapshotRef,
        after,
        change,
        recipe,
      }),
    ).toEqual({ ok: false, reason: "fencing_stale" });
    expect(
      live.commit({
        expectedHead: t0.snapshotRef,
        after,
        change,
        recipe,
      }),
    ).toEqual({ ok: true });
    expect(live.head()).toBe(after.snapshotRef);
  });
});
