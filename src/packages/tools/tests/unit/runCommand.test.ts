...
describe("runCommand", () => {
  ...
  it("rejects commands blocked by sandbox", async () => {
    await expect(runCommand({ command: "rm -rf /" }, { enabled: true }, "/tmp")).rejects.toThrow(
      "denyList",
    );
  ...
  it("resolves with stdout on success", async () => {
    ...
    const output = await runCommand({ command: "echo hello" }, { enabled: true }, "/work");
  ...
  it("includes stderr and exit code in rejected output", async () => {
    ...
    await expect(runCommand({ command: "fail cmd" }, { enabled: true }, "/work")).rejects.toThrow(
      "[stderr]",
    );
  });
  ...
  it("rejects on spawn error", async () => {
    ...
    await expect(runCommand({ command: "missing" }, { enabled: true }, "/work")).rejects.toThrow(
      "spawn failed",
    );
  });
  ...
  it("rejects when command times out", async () => {
    ...
    const pending = runCommand({ command: "sleep 999", timeoutMs: 50 }, { enabled: true }, "/work");
  ...
  it("uses config defaults for timeout and output size", async () => {
    ...
    await runCommand(
      { command: "ok" },
      { enabled: true, timeoutMs: 5000, maxOutputSize: 2048 },
      "/work",
    );
  ...
  it("formats output with only stderr", async () => {
    ...
    await expect(runCommand({ command: "warn cmd" }, { enabled: true }, "/work")).rejects.toThrow(
      "[exit code: 1]",
    );
  });
});
