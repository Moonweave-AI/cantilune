...
| P1b proof (or fallback) | **The finite request/accept operational theorem is implemented_unverified.** The kernel-built chain covers alpha/structural finite-control late-π, native one-step erasure, all four genuine sync/close nominal-incidence cases, exact requesting residual reflection, complete/established classification, and an unconditional `pi_ra_certificate` over the unfiltered structural strong-late LTS. A fresh full local CI/axiom audit and adversarial implementation review pass; immutable provenance and independent review remain mandatory. The separate FMS denotational line is still incomplete: a genuine finite nonempty Hoare Monad with continuous Kleisli laws exists, but it has no empty deadlock or separate divergence; neither exact nor complete availability has an inhabitant, and the all-ωCPO Abramsky powerdomain, domain solution, source-identified Table-2 restriction maps, all-world action bridge, adequate hiding, and full abstraction remain open. **Theory: P1b operational implemented_unverified (needs commit + review); FMS denotational incomplete** |
...
| Bind P1b implemented_unverified result to immutable commit and obtain independent process-semantics + Lean reviewer approval | DRI + process-semantics reviewer + Lean reviewer | Pre-FCP | this RFC §4.2 |
...
4. ⚠ P1b request/accept operational (implemented_unverified; needs immutable commit + review)
